...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			int idx = r.getDestination().getGreen_light_index();
			if (idx != -1 && r.equals(r.getDestination().getIncoming_roads().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getTotal_contamination()) / (1.0 + (double) r.getContamination_alarm_limit()));
